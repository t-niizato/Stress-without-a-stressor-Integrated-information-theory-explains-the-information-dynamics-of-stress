function [] = DatatoFolder(data,name)
%フォルダにデータを入れる関数：データとデータ種類
% PHI, SumPHI, CompType, CompNum (Name)
% CompTypeは別形式
% データはセル形式で受け付ける：出力は空

if name == "PHI" || name == "SumPHI" || name == "CompNum"
    for ind = 1:18
...
elseif name == "CompType"
    ...
    for ind = 1:18
        ...
        writematrix(bit_seq(data{ind,3}), name1)
        writematrix(bit_seq(data{ind,1}), name2)
        writematrix(bit_seq(data{ind,2}), name3)
